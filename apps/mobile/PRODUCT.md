...
- GitHub connects on the host: live hosts take a personal access token that the bridge stores locally (pasted from the app or set as `PRIME_POCKET_GITHUB_TOKEN`), and `--demo` serves a mock catalog for demos/e2e without credentials. Repositories and branches come from that catalog.
...
- GitHub integration is host-side: the personal access token lives on the bridge and is never stored by the app or in the cloud. Mock mode is the default in demo for tests and recordings without credentials. Browser OAuth is not implemented.
